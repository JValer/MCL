   ...
   for (int i = 0; i < 24; i++) {
                     if (IS_BIT_SET32(pattern_rec.lockPatterns[tracknumber], i)) {
                                 ...
                                 for (int s = 0; s < 64; s++) {
                                   ...
                                   if ((pattern_rec.locks[idx][s] != 255) && (pattern_rec.locks[idx][s] >= 0)) {
                                      step[n] = s;
                                      param_number[n] = i;
...
file.open(true);
   ...
   fat_resize_file(file.fd, (uint32_t) sizeof(MDTrack) * (uint32_t) 130 *(uint32_t) 16);    
...
if (patternswitch == 5) {
         ...
         if (pattern_rec.fromSysex(MidiSysex.data + 5, MidiSysex.recordLen - 5)) {    
          ...
          /*Initialize locks*/
                              for (uint8_t g = 0; g < (24); g++) { templocks[g] = 255; }
                                       ...
                                       for (int m = 0; m < 24; m++) {
                                             if (IS_BIT_SET32(pattern_rec.lockPatterns[curtrack], m)) {
                                                  ...
                                                  int8_t value = pattern_rec.locks[idx][ i];
                                                   ...
                                                   if (value != 255) { templocks[m] = value; }
                                             }
                                ...
                                for (int m = 0; m < 24; m++) {
	                                      ...
	                                      int8_t param = m;
                                              int8_t value = pattern_rec.locks[idx][pattern_rec.patternLength - i - 1];
                                               ...
                                               if (value != 255) { pattern_rec.addLock(curtrack, i, param, value); }
                                           }
                     ...
                     /*Clear the locks*/       
                             ...
                             /*Set the new step to match the loop step*/
	                            ...
	                            int8_t value = pattern_rec.locks[idx][n];
                                   if (value != 255) { pattern_rec.addLock(curtrack, i+n, param, value); }
                                           }
                                 }
                             }  
...
/*If patternswitch == 0, the pattern receiveed is for storing track data*/
         ...
         if (pattern_rec.fromSysex(MidiSysex.data + 5, MidiSysex.recordLen - 5)) {
            ...
            for (int i = 0; i < store_n_tracks; i++) {

                                bool n = store_track_inGrid(curtrack + i, cur_col + i, cur_row);
           ...
           if (pattern_rec.fromSysex(MidiSysex.data + 5, MidiSysex.recordLen - 5)) {
                ...
                /*Send a quick sysex message to get the current selected track of the MD*/       
                  ...
                  */
                    ...
                    int i = 0;
                    while ((trackwrite_queue[i][0] != 255) && (i < 16)) {

                           if (trackposition == true) {
                             /*-1 here because encoder values needed to be scaled up to prevent NULL = 0 bug*/
                                place_track_inpattern(trackwrite_queue[i][0], trackwrite_queue[i][0], trackwrite_queue[i][1]);
                                  }
                           else if (curtrack + i < 16) {
                                place_track_inpattern(curtrack + i,trackwrite_queue[i][0], trackwrite_queue[i][1]);
                           }
                      i++;
                  ...
                  pattern_rec.setPosition(pattern_rec.origPosition);
                   ...
                   if (writepattern != MD.currentPattern) {
                    pattern_rec.setKit(patternload_param3.getValue() - 1);
                   }
                  ...
                  */
                     ...
                     if ((writepattern != -1) && (writepattern != MD.currentPattern)) {
                                          kit_new.origPosition = patternload_param3.getValue() - 1;
...
*/
void write_tracks_to_md(int patternx) { 
        patternswitch = 1;

        writepattern = patternx;
...
void trackwrite_queue_init() {
	for (int i = 0; i < 16; i++) {
        trackwrite_queue[i][0] = 255;
        trackwrite_queue[i][1] = 255;
...
void puton_trackwrite_queue(int column, int row) {
        ...
        if (load_track(column, row)) {
           if (temptrack.active == true) {
              ...
              int i = 0;
              while ((i < 16) && (trackwrite_queue[i][0] != 255)) { i++; }
...
void clear_step_locks(int curtrack, int i) {
                             for (uint8_t p = 0; p < 24; p++) {
	                          ...
	                          if (idxn != -1) { pattern_rec.locks[idxn][i] = 255;  }
	                        }
}
...
void PatternLoadEncoder::displayAt(int encoder_offset) {
              ...
              if (curpage == 3) { GUI.put_string_at(0,"Capture Pattern:"); }
              else if (curpage == 4) { GUI.put_string_at(0,"Write Pattern:"); }
               GUI.setLine(GUI.LINE2);
...
bool handleEvent(gui_event_t *evt) {
   ...
   //            curpage = 0;
    ...
    //          }
          if (EVENT_PRESSED(evt, Buttons.ENCODER1)) {
           ...
           if (!EVENT_PRESSED(evt, Buttons.BUTTON3) && !EVENT_PRESSED(evt, Buttons.BUTTON4) && !EVENT_RELEASED(evt, Buttons.BUTTON3) && !EVENT_RELEASED(evt, Buttons.BUTTON4)) {
  ...
  //  if (EVENT_PRESSED(evt, Buttons.BUTTON3) || EVENT_PRESSED(evt, Buttons.BUTTON4)) {
   ...
   //            curpage = 0;
    ...
    //          }
         /* if (EVENT_PRESSED(evt, Buttons.ENCODER1)) {
                    for (int i = 0; i < 8; i++) { 
                          puton_trackwrite_queue(i,param2.getValue());
                      }
                      trackposition = TRUE; write_tracks_to_md(-1);
                       GUI.setPage(&page);
                       curpage = 0;
                       return true;
           } 
    ...
    //       } 
        ...
        //               curpage = 0;
           ...
           if (!EVENT_PRESSED(evt, Buttons.BUTTON1) && !EVENT_PRESSED(evt, Buttons.BUTTON2) && !EVENT_RELEASED(evt, Buttons.BUTTON1) && !EVENT_RELEASED(evt, Buttons.BUTTON2)) {
                for (int i = 0; i < 16; i++) { 
            ...
            write_tracks_to_md(patternload_param2.cur + (patternload_param1.cur * 16));
     ...
     if (EVENT_PRESSED(evt, Buttons.ENCODER1) && BUTTON_DOWN(Buttons.BUTTON4)) {
      ...
      if ((EVENT_PRESSED(evt, Buttons.BUTTON1) && BUTTON_DOWN(Buttons.BUTTON4)) || (EVENT_PRESSED(evt, Buttons.BUTTON4) && BUTTON_DOWN(Buttons.BUTTON1) ))
        ...
        if ((EVENT_PRESSED(evt, Buttons.BUTTON1) && BUTTON_DOWN(Buttons.BUTTON2)) || (EVENT_PRESSED(evt, Buttons.BUTTON2) && BUTTON_DOWN(Buttons.BUTTON1) ))
       {
        ...
        if ((EVENT_PRESSED(evt, Buttons.BUTTON3) && BUTTON_DOWN(Buttons.BUTTON4)) || (EVENT_PRESSED(evt, Buttons.BUTTON4) && BUTTON_DOWN(Buttons.BUTTON3) ))
       {
  ...
  if (BUTTON_DOWN(Buttons.ENCODER4) && (BUTTON_DOWN(Buttons.BUTTON3)))  {
     ...
     return true;
         ...
         /*Store encoder press in a queue*/
         if (BUTTON_UP(Buttons.BUTTON1)) {
           
    
              if (EVENT_PRESSED(evt, Buttons.ENCODER1)) {
                

                puton_trackwrite_queue(param1.getValue(), param2.getValue());
                return true;
               }
               if (EVENT_PRESSED(evt, Buttons.ENCODER2)) {
                 puton_trackwrite_queue(param1.getValue() + 1, param2.getValue());
                 return true;
               }
               if (EVENT_PRESSED(evt, Buttons.ENCODER3)) {
                  puton_trackwrite_queue(param1.getValue() + 2, param2.getValue());
                  return true;
               }
               if (EVENT_PRESSED(evt, Buttons.ENCODER4)) {
                  puton_trackwrite_queue(param1.getValue() + 3, param2.getValue());
                  return true;
               }
          
               /*If all the encoders are released then write the tracks to the MD in sequential order*/
         
               if ((BUTTON_UP(Buttons.BUTTON1) && BUTTON_UP(Buttons.BUTTON2) && BUTTON_UP(Buttons.BUTTON3) && BUTTON_UP(Buttons.BUTTON4)) && 
                   (EVENT_RELEASED(evt, Buttons.ENCODER1) || EVENT_RELEASED(evt, Buttons.ENCODER2) || EVENT_RELEASED(evt, Buttons.ENCODER3) || EVENT_RELEASED(evt, Buttons.ENCODER4))
               && ( BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) ) )
               {
                  /*Check to see if there is more than one track on the queue*/
           
                  /*If there are tracks in the queue, write them*/
                  if (trackwrite_queue[0][0] != 255) {  trackposition = TRUE; write_tracks_to_md(-1); }
                  /*Re-initialise the encoder queue*/
                  return true;
               }
         }   
         
                if ((BUTTON_UP(Buttons.BUTTON1) && BUTTON_DOWN(Buttons.BUTTON2) && BUTTON_UP(Buttons.BUTTON3) && BUTTON_UP(Buttons.BUTTON4)) && 
                   (EVENT_RELEASED(evt, Buttons.ENCODER1) || EVENT_RELEASED(evt, Buttons.ENCODER2) || EVENT_RELEASED(evt, Buttons.ENCODER3) || EVENT_RELEASED(evt, Buttons.ENCODER4))
               && ( BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) && BUTTON_UP(Buttons.ENCODER1) ) )
               {
                   /*If there are tracks in the queue, write them*/
                  if (trackwrite_queue[0][0] != 255) { trackposition = FALSE; write_tracks_to_md(-1); }
                 /*Re-initialise the encoder queue*/
                  return true;
              }
